...
public sealed class SmartMeterFleet
{
    ...
    public MeterSnapshot Create(CreateMeterRequest request)
    {
        ...
        if (request.NominalVoltage is < 50 or > 500) throw new ArgumentOutOfRangeException(nameof(request), "Nominal voltage must be between 50 V and 500 V.");
        if (request.BaseLoadKw is <= 0 or > 10_000) throw new ArgumentOutOfRangeException(nameof(request), "Base load must be between 0 and 10,000 kW.");
        if (request.NominalPowerFactor is < 0.1 or > 1) throw new ArgumentOutOfRangeException(nameof(request), "Power factor must be between 0.1 and 1.0.");
