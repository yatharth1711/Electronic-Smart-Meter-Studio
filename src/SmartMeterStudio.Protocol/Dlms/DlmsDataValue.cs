...
{
    Null = 0, Array = 1, Structure = 2, Boolean = 3, BitString = 4, DoubleLong = 5, DoubleLongUnsigned = 6,
...
public sealed record DlmsDataValue(DlmsDataType Type, object? Value)
{
    ...
    public static DlmsDataValue Bits(int length, ReadOnlySpan<byte> value) => new(DlmsDataType.BitString, new DlmsBitString(length, value.ToArray()));
}

/// <summary>Bit-string payload with an explicit number of valid bits (A-XDR tag 4).</summary>
public sealed record DlmsBitString(int Length, byte[] Bytes);
